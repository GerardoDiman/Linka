...
async function handleDatabases(req, res) {
  try {
    ...
    console.log('🔍 [VERCEL] Buscando bases de datos con token del usuario...');
    ...
    console.log(`✅ [VERCEL] Encontradas ${response.results.length} bases de datos`);
    ...
    const enrichedDatabases = await Promise.all(
      response.results.map(async (db) => {
        try {
          ...
          console.warn(`[VERCEL] No se pudo obtener detalles de ${db.id}:`, error.message);
          return db;
        }
      })
    );
    ...
    console.error('❌ [VERCEL] Error en la API:', error);
...
async function handleSingleDatabase(req, res, databaseId) {
  try {
    ...
    console.log(`🔍 [VERCEL] Obteniendo detalles de base de datos: ${databaseId}`);
    ...
    console.log(`✅ [VERCEL] Detalles obtenidos para: ${database.title?.[0]?.plain_text || databaseId}`);
    ...
    console.error(`❌ [VERCEL] Error obteniendo base de datos ${databaseId}:`, error);
...
async function handleTestConnection(req, res) {
  try {
    ...
    console.log('🧪 [VERCEL] Probando conexión con Notion...');
    ...
    console.log('✅ [VERCEL] Conexión exitosa:', botInfo.name || 'Bot sin nombre');
    ...
    console.error('❌ [VERCEL] Error al probar conexión:', error);
...
// Exportar la función principal para Vercel
module.exports = async (req, res) => {
  ...
  // Enrutar requests
  if (method === 'GET' && path === '/api/notion/databases') {
    await handleDatabases(req, res);
  } else if (method === 'GET' && path === '/api/notion/test-connection') {
    await handleTestConnection(req, res);
  } else if (method === 'GET' && path.startsWith('/api/notion/databases/')) {
    // Manejar requests específicos de base de datos: /api/notion/databases/{id}
    const databaseId = path.split('/api/notion/databases/')[1];
