...
function App() {
  ...
  useEffect(() => {
    ...
    // Función para calcular puntos de conexión óptimos
  const calculateOptimalConnectionPoints = useCallback((sourcePos: {x: number, y: number}, targetPos: {x: number, y: number}) => {
    ...
    if (distance < 1) {
      return { sourceHandle: 'right-center', targetHandle: 'left-center' }
    }
    ...
    // Convertir a nuevos IDs híbridos
    sourceHandle = sourceHandle + '-s' // Agregar sufijo source
    targetHandle = targetHandle + '-t' // Agregar sufijo target
  ...
  }, [nodes, calculateOptimalConnectionPoints])
